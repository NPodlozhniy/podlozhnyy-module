...
def beauty_hist(df, feature, target, n):
    ...
    n: Кол-во bin-ов 
    """
    df2 = pd.melt(df[[feature, target]], id_vars=target, value_vars=[feature], value_name='value')
    bins = np.linspace(df2.value.min(), df2.value.max(), n)
    g = sns.FacetGrid(df2, col="variable", hue=target, palette='rainbow', col_wrap=2, size = 10)
...
def iv_agg(df, features, target, num_bucks=[10, 10]):
    ...
    """
    index = make_bucket(df[[features[-1]]], features[-1], num_buck=num_bucks[-1])['bucket'].values
    columns = []

    for i, feature in enumerate(features[:-1]):
        columns.append(make_bucket(df[[feature]], feature, num_buck=num_bucks[i])['bucket'].values)
...
def distribution(df, feature, date, num_buck=10, date_freq='Q'):
    ...
    data = hv.Dataset(agg, kdims=['bucket', date], vdims=['objects_rate', 'obj_rate_b', 'obj_rate_u'])
...
def HL(target, predict, num_buck=10):
    ...
    return int(sum((data.predict - data.bad_rate) ** 2 / (data.predict * (1 - data.predict)) * data.obj_cnt))
    ...
    def double_exponential_smoothing(self):
        ...
        for i in range(len(self.series) + self.n_preds):
            ...
            if i >= len(self.series): # прогнозируем
                ...
                val = self.series[i]                
                prev_level, level = level, self.alpha * val + (1 - self.alpha) * (level + trend)
    ...
    def triple_exponential_smoothing(self):
        ...
        for i in range(len(self.series) + self.n_preds):
            ...
            if i >= len(self.series): # прогнозируем
                ...
                val = self.series[i]
                prev_smooth, smooth = smooth, self.alpha * (val - seasonals[i%self.slen]) + (1 - self.alpha) * (smooth + trend)
                trend = self.beta * (smooth - prev_smooth) + (1 - self.beta) * trend
                seasonals[i%self.slen] = self.gamma * (val - smooth) + (1 - self.gamma) * seasonals[i%self.slen]

                # Отклонение рассчитывается в соответствии с алгоритмом Брутлага
                prev_deviation = deviations[i%self.slen]
                deviations[i%self.slen] = self.gamma * np.abs(self.series[i] - self.result[i]) + (1 - self.gamma) * prev_deviation
...
def timeseriesCVscore(x, data, r=0, method='HoltWinters', slen=7):
    ...
    def weighted_mse(actual, predictions, r):
        ...
        return np.mean(((np.array(actual) - np.array(predictions)) ** 2) * np.array(weights))
    ...
    for train, test in tscv.split(values):
        ...
        if method == 'HoltWinters':
            model = HoltWinters(series=values[train], slen=slen, alpha=x[0], beta=x[1], gamma=x[2], n_preds=len(test))
            model.triple_exponential_smoothing()

        if method == 'Holt':
            model = HoltLinearTrend(series=values[train], alpha=x[0], beta=x[1], n_preds=len(test))
            model.double_exponential_smoothing()
 
        predictions = model.result[-len(test):]
        actual = values[test]

    # Можно считать обыычный MSE или взвесить и дать больший вес свежим значением
...
def plotHolt(model, dataset, target, predict_interval, xlim=None):
    ...
    if len(model.result) > len(dataset):
        dataset = pd.concat([dataset, pd.DataFrame(np.array([np.NaN] * predict_interval), columns=[target])])
    plt.figure(figsize=(25, 10))
    plt.plot(model.result, "b", label = "Model")
    plt.plot(dataset[target].values, "g", label = "Actual")
    plt.axvspan(len(dataset) - predict_interval - 1, len(dataset), alpha=0.5, color='lightgrey')
    plt.grid(True)
    plt.axis('tight')
...
def plotHoltWinters(model, dataset, target, predict_interval, xlim=None):
    ...
    Anomalies[dataset[target].values < model.LowerBond] = dataset[target].values[dataset[target].values < model.LowerBond]
    Anomalies[dataset[target].values > model.UpperBond] = dataset[target].values[dataset[target].values > model.UpperBond]
    plt.figure(figsize=(25, 10))
    plt.plot(model.result, "b", label = "Model")
...
def stat_difference_by_flg(df, feature, target, name, flg, num_buck=10):
    ...
    if df[feature].dtype == 'O':
        ...
        return before.join(after.set_index('bucket'), on = 'bucket', rsuffix='_after') \
            ...
            .assign(AR_decrease_overall = lambda x: x.AR_decrease_mean * x.feature_cnt_after / after.feature_cnt.sum()) \
            [['bucket', 'AR_decrease_overall', 'AR_decrease_mean', 'AR_decrease_min', 'AR_decrease_max']] \
...
# Считаем среднюю разницу и доверительный интервал для этой средней
        return before.join(after.set_index('bucket'), on = 'bucket', rsuffix='_after')\
            ...
            .assign(AR_decrease_min = lambda x: x.AR_decrease_mean - 1.96 * x.AR_decrease_std,
                    AR_decrease_max = lambda x: x.AR_decrease_mean + 1.96 * x.AR_decrease_std)\
            .assign(AR_decrease_overall = lambda x: x.AR_decrease_mean * x.feature_cnt_after / after.feature_cnt.sum())\
            [['bucket', 'feature_avg', 'feature_avg_after', 'AR_decrease_overall', 'AR_decrease_mean', 'AR_decrease_min', 'AR_decrease_max']]\
